...
async def bigspam(Lion):
    if not Lion.text[0].isalpha() and Lion.text[0] not in ("/", "#", "@", "!"):
        Lion_msg = Lion.text
        Lion_count = int(Lion_msg[9:13])
        Lion_spam = str(Lion.text[13:])
        for i in range(1, Lion_count):
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what Lion iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add_info(
  "Spammers Commands"
).add_warning(
